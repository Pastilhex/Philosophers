...
/*   Updated: 2023/09/13 07:10:43 by ialves-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_eat(t_philo *p)
{
	...
	{
		if (p->link_b->nbr_philos > 1 && check_for_dead_philos(p) != true && pthread_mutex_lock(p->right) == 0 && check_deads_or_meals(p) != true)
		{
...
{
	if (p->link_b->nbr_philos > 1 && check_for_dead_philos(p) != true && check_deads_or_meals(p) != true)
	{
...
void	*routine(void *arg)
{
	...
	while (check_for_dead_philos(p) != true && check_deads_or_meals(p) != true)
	{
		handle_eat(p);
		handle_sleep(p);
		handle_think(p);
